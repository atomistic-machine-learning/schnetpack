...
class OrcaParser:
    ...
    file_extensions = {
        Properties.forces: '.engrad',
        Properties.hessian: '.oinp.hess'
    }
    ...
    def _parse_molecule(self, datafile):
        ...
        if self.hessian_parser is not None:
            hessian_file = os.path.splitext(datafile)[0] + self.file_extensions['hessian']
            if not os.path.exists(hessian_file):
                ...
                for p in hessian_properties:
                    if p is None:
                        ...
                        properties[p] = format_polarizability_derivatives(hessian_properties[p])
                    else:
...
class OrcaFormatter:
    ...
    def __init__(self, position, stop=None, datatype='vector',
                 converter=float, skip_first=None, unit=None, default=None):
...
class OrcaMainFileParser(OrcaOutputParser):
    properties = [
        'atoms', Properties.forces, Properties.energy, Properties.dipole_moment, Properties.polarizability,
        Properties.shielding]
    ...
    formatters = {
        'atoms': (OrcaFormatter(0, converter=str), OrcaFormatter(1, stop=4, unit=1.0 / units.Bohr)),
    ...
    def __init__(self, properties=None):
        ...
        parsers = {p: OrcaPropertyParser(self.starts[p], self.stops[p], formatters=self.formatters[p]) for p in
                   to_parse}
...
class OrcaHessianFileParser(OrcaMainFileParser):
    properties = [Properties.hessian, Properties.dipole_derivatives, Properties.polarizability_derivatives]
