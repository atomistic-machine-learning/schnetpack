...
class OrcaParser:
    ...
    Args:
        ...
        filter (dict, optional): Dictionary giving the name of a property and a threshold value. Entries in
                                 the output files with values exceeding the threshold in magnitude are discarded.
                                 This can be used to e.g. screen for numerical noise in implicit solvent computations,
                                 etc.
                    ...
                    if self.filter is not None:
                        ...
                        for p in self.filter:
                            if np.linalg.norm(properties[p]) > self.filter[p]:
                                filtered = True
                                log.info(f"Filtered output {file} due to {p}")
                        if not filtered:
