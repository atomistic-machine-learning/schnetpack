...
class SpkCalculator(Calculator):
    ...
    def __init__(
        self, ml_model, environment_provider=SimpleEnvironmentProvider(), **kwargs
    ):
...
def load_model(modelpath, cuda=True):
    ...
    if isinstance(model.output_modules, Iterable):
        ...
        if isinstance(model.output_modules, Energy) or isinstance(
            model.output_modules, ElementalEnergy
        ):
            has_energy = True
