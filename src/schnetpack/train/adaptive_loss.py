...
class Distribution:
    ...
    def __init__(self):
        ...
        spline_file = os.path.join(
            os.path.dirname(__file__), "ressources/partition_spline_for_robust_loss.npz"
        )
...
class AdaptiveLossFunction(torch.nn.Module):
    ...
    def __init__(
        ...
        if alpha_lo == alpha_hi:
            ...
            # a constant.
            self.fixed_alpha = (
                alpha_lo.unsqueeze(0).unsqueeze(0).repeat(1, self.num_dims)
            )
            ...
            if alpha_init is None:
                alpha_init = torch.as_tensor((alpha_lo + alpha_hi) / 2.0)
            ...
            self.register_parameter(
                "latent_alpha",
                torch.nn.Parameter(latent_alpha_init_1, requires_grad=True),
            )
        ...
        if scale_lo == scale_init:
            ...
            self.fixed_scale = (
                scale_init.unsqueeze(0).unsqueeze(0).repeat(1, self.num_dims)
            )
