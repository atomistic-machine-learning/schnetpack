    ...
    def configure_optimizers(self):
        ...
        if self.scheduler_cls:
            ...
            if self.trainer.val_check_interval < 1.0:
                warnings.warn(
                    "Learning rate scheduling is set to occur after the epoch ends. To enable scheduling before the epoch end, "
                    "please set the `val_check_interval` parameter to a value greater than 1.0, which indicates the number of training "
                    "steps after which the model should be validated."
                )
