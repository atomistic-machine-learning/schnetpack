...
class SpecialOption(argparse.Action):
    ...
    def __init__(self, option_strings, const, dest, type=type, default=False, required=False, help=None):
...
def get_parser():
    ...
    main_parser.add_argument('--cuda', help='Set flag to use GPU(s)', action='store_true')

    # Optimization:
    main_parser.add_argument('--optimize', const=1000, action=SpecialOption, type=int,
                             help='Optimize geometry. A default of 1000 steps is used.')

    # Normal modes
    main_parser.add_argument('--normal_modes', help='Compute normal modes', action='store_true')

    # Molecular dynamics options
    main_parser.add_argument('--equilibrate', const=10000, action=SpecialOption, type=int,
                             help='Equilibrate molecule for molecular dynamics. A default of 10000 steps is used.')
    main_parser.add_argument('--production', const=10000, action=SpecialOption, type=int,
                             help='Production molecular dynamics (default: %(default)s)')
    main_parser.add_argument("--interval", type=int, default=100,
                             help='Log trajectory every N steps (default: %(default)s)')
    main_parser.add_argument("--time_step", type=float, default=0.5, help='Timestep in fs (default: %(default)s)')

    # Temperature and bath options
    main_parser.add_argument('--temp_init', type=float, default=300.0, help='Initial temperature used for molecular'
                                                                            ' dynamics in K. (default: %(default)s)')
    main_parser.add_argument('--temp_bath', type=float, default=None, help='Temperature of bath. If None is given, NVE'
                                                                           ' dynamics are run. (default: %(default)s)')

    main_parser.add_argument('--single_point', action='store_true', help='Perform a single point prediction on the '
...
if __name__ == '__main__':
    ...
    logging.info("The model you built has: {:d} parameters".format(spk.utils.compute_params(ml_model.model)))

    # Initialize the ML ase interface
    ml_calculator = spk.md.AseInterface(args.molecule_path, ml_model, args.simulation_dir)
    ...
    if args.normal_modes:
        if not args.optimize:
            logging.warning('Computing normal modes without optimizing the geometry makes me a sad schnetpack...')
    ...
    if args.equilibrate:
        ...
        ml_calculator.init_md("equilibration", time_step=args.time_step, interval=args.interval,
                              temp_bath=args.temp_bath, temp_init=args.temp_init)
        ml_calculator.run_md(args.equilibrate)

    if args.production:
        logging.info("Running production dynamics...")
        ml_calculator.init_md("production", time_step=args.time_step, interval=args.interval, temp_bath=args.temp_bath,
