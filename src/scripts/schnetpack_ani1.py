...
def get_parser():
    ...
    train_parser.add_argument('datapath',
                              help='Path / destination of ANI1 dataset directory')
    ...
    wacsf_parser.add_argument('--centered', action='store_true',
                              help='Use centered Gaussians for radial functions')
    wacsf_parser.add_argument('--crossterms', action='store_true',
                              help='Use crossterms in angular functions')
    wacsf_parser.add_argument('--behler', action='store_true',
                              help='Switch to conventional ACSF')
    wacsf_parser.add_argument('--elements', default=['H', 'C', 'N', 'O', 'F'],
                              nargs='+',
    ...
    train_subparsers.add_parser('wacsf', help='wACSF help',
                                parents=[train_parser, wacsf_parser])
    ...
    eval_subparsers.add_parser('wacsf', help='wACSF help',
                               parents=[eval_parser, wacsf_parser])
...
def evaluate(args, model, property, train_loader, val_loader, test_loader,
    ...
    if 'train' in args.split:
        ...
        results += evaluate_dataset(metrics, model,
                                    train_loader, device)
    ...
    np.savetxt(os.path.join(args.modelpath, 'evaluation.txt'), results,
...
def get_model(args, atomref=None, mean=None, stddev=None, train_loader=None,
    ...
    if args.model == 'schnet':
        ...
        representation = spk.representation.BehlerSFBlock(args.radial,
                                                          args.angular,
                                                          zetas=set(
                                                              args.zetas),
                                                          ...
                                                          elements=elements)
        logging.info(
            "Using {:d} {:s}-type SF".format(representation.n_symfuncs,
                                             sfmode))
    ...
    logging.info(
        "The model you built has: %d parameters" % compute_params(model))
...
if __name__ == '__main__':
    ...
    ani1 = spk.datasets.ANI1(args.datapath, download=True,
    ...
    data_train, data_val, data_test = ani1.create_splits(*args.split,
