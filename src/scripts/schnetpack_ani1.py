...
def get_parser():
    ...
    schnet_parser.add_argument('--features', type=int, help='Size of atom-wise representation',
                               default=256)
    schnet_parser.add_argument('--interactions', type=int, help='Number of interaction blocks',
                               default=6)
...
def evaluate(args, model, train_loader, val_loader, test_loader, device):
    ...
    np.savetxt(os.path.join(args.modelpath, 'evaluation.txt'), results, header=header, fmt='%.5f')
    ...
    if args.model == 'schnet':
        ...
        if args.standardize and mode == 'train':
            if train_loader is None:
                ...
                representation = spk.representation.StandardizeSF(representation, train_loader,
                                                                  cuda=args.cuda)
...
if __name__ == '__main__':
    ...
    data_train, data_val, data_test = ani1.create_splits(*args.split, split_file=split_path)
