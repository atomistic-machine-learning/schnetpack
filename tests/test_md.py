...
def get_thermostat_defaults(thermostat):
    ...
    if thermostat == 'gle':
        thermostat_config['gle_input'] = os.path.join(os.path.dirname(os.path.realpath(__file__)),
...
class TestSacred:
    ...
    def test_run_md(self, md_config, simulation_dir, molecule_path, model_path, md_thermostats, md_integrator):
        ...
        md_config['dynamics']['load_system_state'] = os.path.join(simulation_dir, 'checkpoint.chk')
        md = MDSimulation(md_config)
        md.save_config()
        md.run()

        # Test restart
        md_config['dynamics']['restart'] = os.path.join(simulation_dir, 'checkpoint.chk')
        md = MDSimulation(md_config)
        md.save_config()
        md.run()

    @pytest.mark.filterwarnings("ignore")
    def test_run_rpmd(self, md_config, simulation_dir, molecule_path, model_path, rpmd_thermostats, rpmd_integrator):
        md_config = setup_basic_md(md_config, simulation_dir, molecule_path, model_path)
