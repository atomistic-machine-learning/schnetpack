...
def two_site_crystal():
    return Atoms([6, 6], positions=[[0., 0., 0.], [.1, 0., 0.]], cell=np.eye(3), pbc=True)
...
def test_single_atom(single_atom, simple_env, ase_env):
    nbh_simple, offsets_simple = simple_env.get_environment(0, single_atom)
    nbh_ase, offsets_ase = ase_env.get_environment(0, single_atom)
...
def test_two_atoms(two_atoms, simple_env, ase_env):
    nbh_simple, offsets_simple = simple_env.get_environment(0, two_atoms)
    nbh_ase, offsets_ase = ase_env.get_environment(0, two_atoms)
...
def test_single_site_crystal_small_cutoff(crystal, simple_env, ase_env):
    ...
    nbh_simple, offsets_simple = simple_env.get_environment(0, crystal)
