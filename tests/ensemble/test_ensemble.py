...
def test_absolute_uncertainty(dummy_atoms):
    calc = make_test_calc(
        models=[DummyModel(), DummyModel()], uncertainty_fn=AbsoluteUncertainty()
    )
    calc.calculate(dummy_atoms)
...
def test_relative_uncertainty(dummy_atoms):
    calc = make_test_calc(
        models=[DummyModel(), DummyModel()], uncertainty_fn=RelativeUncertainty()
    )
    calc.calculate(dummy_atoms)
    assert isinstance(calc.results["uncertainty"], numbers.Number)
...
def test_relative_uncertainty_zero_for_identical(dummy_atoms):
    calc = make_test_calc(
        models=[ConstantModel(), ConstantModel()], uncertainty_fn=RelativeUncertainty()
    )
    calc.calculate(dummy_atoms)
...
def test_zero_uncertainty_for_identical_models(dummy_atoms):
    calc = make_test_calc(
        models=[ConstantModel(), ConstantModel()], uncertainty_fn=AbsoluteUncertainty()
    )
    calc.calculate(dummy_atoms)
    assert np.isclose(calc.results["uncertainty"], 0.0)
...
def test_invalid_uncertainty_return(dummy_atoms):
    calc = make_test_calc(
        models=[DummyModel(), DummyModel()], uncertainty_fn=InvalidUncertainty()
    )
    calc.calculate(dummy_atoms)
...
def test_combined_uncertainty(calculator, dummy_atoms):
    ...
    forces_uncertainty = np.sqrt(
        force_x_uncertainty**2 + force_y_uncertainty**2 + force_z_uncertainty**2
    )
    ...
    stress_uncertainty = np.sqrt(
        stress_xx_uncertainty**2 + stress_yy_uncertainty**2 + stress_zz_uncertainty**2
    )

    # Calculate the combined uncertainty using RMS method
    combined_uncertainty = np.sqrt(
        energy_uncertainty**2 + forces_uncertainty**2 + stress_uncertainty**2
    )
    ...
    assert np.isclose(
        combined_uncertainty, 0.3241, atol=1e-3
    )  # Change value as per your calculation
    assert isinstance(energy, numbers.Number)
...
def test_fake_uncertainty(calculator, dummy_atoms):
    ...
    assert np.isclose(
        uncertainty_results["AbsoluteUncertainty"], 0.0, atol=1e-2
